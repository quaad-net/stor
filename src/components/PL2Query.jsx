...
const PL2Query = memo(function PL2Query(props) {
    ...
    try{
      ...
      await fetch(`${aiUrl}/stor-pl-img`, 
        {
          ...
          body: JSON.stringify({img: imageData.split(',')[1], user: props.user.email, token: props.token})
        }
      )
      .then((res)=>{
        return res.json()
      ...
      .then((res)=>{
        if(res.message == 'Complete'){
            const vendorItems = res.items.itemNumbers;
            let itemsToString = '';
            vendorItems.map((item, index)=>{
                if(index != vendorItems.length -1){
                    itemsToString += item + '---'
                }
        ...
        >
          <AppBar sx={{ position: 'relative'}}>
            <Toolbar>
              ...
              {imgCaptured && !imgSent && !loading ?
                // Add after-capture component here
                <></> 
                :
                <></>
              }
          ...
          <List>
            <ListItemButton disabled>
              <ListItemText primary={loading ? 'Fetching Data...' : 'Take a snaphot of your packing list or invoice to run query...'} />
            </ListItemButton>
            <Divider />
          </List>
